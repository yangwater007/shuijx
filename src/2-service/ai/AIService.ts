...
const FULL_REVIEW_PROMPT =
  "??A???????????????????????????????????K?/?????????kline/minute_data????????????????????\n\n" +
  "?????-??????market_overview + limit_stats + limit_up_ladder + limit_yesterday_premium + limit_bigloser + concept_ranking + capital_flow(type:sector)\n" +
  "???????sector_analysis(??=????) / broken_limit_up + limit_down(???) / dragon_tiger(?????) / stock_rank(type:gainers)\n" +
  "??????-??????[?]?????kline(code,days:30,adjust:qfq)???+??????????kline????\n\n" +
  "?????- ?????\n" +
  "?????????+????+???+??????????:market_overview?\n" +
  "????? & ???????/??/??/???/??/??????:limit_stats + limit_bigloser + limit_yesterday_premium?\n" +
